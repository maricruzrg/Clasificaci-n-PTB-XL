...
y_probs_ECG = np.load(os.path.join(outputfolder, 'y_probs_ensemble_xresnet.npy'))  
y_true_ECG = np.load(os.path.join(outputfolder, 'ytrue_ensemble_xresnet.npy'))

# Verificar consistencia
assert np.allclose(y_true_bigru, y_true_ECG), " Los y_true no coinciden entre modelos"
y_true = y_true_bigru  # usar uno solo

# ----------------------
# Cargar nombres de clases y thresholds
# ----------------------
mlb_classes = ['CD', 'HYP', 'MI', 'NORM', 'STTC']  # orden correcto


# ----------------------
# Fusión por promedio
# ----------------------
y_probs_fused = (y_probs_bigru + y_probs_ECG) / 2
...
# Guardar resultados
# ----------------------
np.save(os.path.join(outputfolder, 'y_probs_fusedXresbigru.npy'), y_probs_fused)
np.save(os.path.join(outputfolder, 'y_pred_fusedXresbigru.npy'), y_pred_fused)
